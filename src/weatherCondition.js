...
const  weatherCondition = (currentWeather) => {
    ...
    // in ?. format
    const temperature = currentWeather && currentWeather.main && currentWeather.main.temp;
    const windSpeed = currentWeather && currentWeather.wind && currentWeather.wind.speed;
    const gust = currentWeather && currentWeather.wind && currentWeather.wind.gust;




  // conditions that players would want to take note of.
    if (temperature >= 50 && temperature <= 85 && windSpeed <= 18 && (!gust || gust < 20)) {
      return (
        <>
      ...
      return (
        ...
        return (
          ...
          <p>Today isn't optimal but could be playable. It might be too windy though.</p>
          </>
        )
      ...
      return (
        <>
        <img src={caution} alt='caution sign'/>
        <p>Today seems a bit too cold. Dress warmly if you're going outside to play.</p>
        </>
      )
      ...
      return (
        <>
        <img src={caution} alt='caution sign'/>
        <p>Today is really hot. Make sure to stay hydrated and cool if you want to play.</p>
        </>
      )
    } else {
      return (
